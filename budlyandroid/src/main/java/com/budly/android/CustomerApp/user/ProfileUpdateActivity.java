...
public class ProfileUpdateActivity extends BaseActivity implements View.OnClickListener {
	...
	TextView change_profile_photo, change_recomendation_info, change_license_photo, change_recomendation_photo;

	public void onClick(View paramView) {
		switch (paramView.getId()) {
		case R.id.change_license_photo:
			startActivity(new Intent(this, ProfilePictureLicenseUpdateActivity.class));
			break;
		case R.id.change_recomendation_info:
			startActivity(new Intent(this, RecomendationUpdateActivity.class));
			break;
        case R.id.change_recomendation_photo:
                startActivity(new Intent(this, ProfilePictureRecomendationUpdateActivity.class));
                break;
		default:
			break;
		}
			...
			if(!password.getText().toString().equals(password_again.getText().toString())) {
				...
				HttpBasicClientHelper client = new HttpBasicClientHelper(new MyJsonAsyncCallback(){
					...
					public void onFailure(Exception error) {
						...
						Toast.makeText(ProfileUpdateActivity.this, "Update failed", Toast.LENGTH_SHORT).show();
					}
		...
		if (first_name.getText().toString().equals("") || last_name.getText().toString().equals("")
			...
			if(!password.getText().toString().equals(password_again.getText().toString())) {
				...
				client.register(mUser.first_name, mUser.last_name, mUser.phone_number, mUser.email, 
						mUser.password, "customer", mUser.address, mUser.City, mUser.state, mUser.zip, mUser.signup_date);
			}
		}
	}
	...
	protected void onCreate(Bundle paramBundle) {
		...
		setContentView(R.layout.user_activity_profile_update);
		change_license_photo = (TextView) findViewById(R.id.change_license_photo);
//		change_profile_photo = (TextView) findViewById(R.id.change_profile_photo);
        change_recomendation_photo = (TextView) findViewById(R.id.change_recomendation_photo);
        change_recomendation_photo.setOnClickListener(this);
        change_license_photo.setOnClickListener(this);
        change_recomendation_info = (TextView) findViewById(R.id.change_recomendation_info);
        change_recomendation_info.setOnClickListener(this);
//		change_profile_photo.setOnClickListener(this);
		...
		if(mUser.id>0) {
			((EditText) findViewById(R.id.txt_number)).setText(mUser.phone_number);
		}
	}
